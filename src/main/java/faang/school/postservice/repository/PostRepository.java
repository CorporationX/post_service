...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p " +
            "WHERE p.authorId = :userId AND p.createdAt < :lastPostCreatedAt " +
            "ORDER BY p.createdAt DESC")
    List<Post> findNextBatch(@Param("userId") Long userId,
                             Pageable pageable);
    ...
    @Query("SELECT p FROM Post p WHERE p.authorId IN :postAuthorIds " +
            ...
            "ORDER BY p.createdAt DESC")
    List<Post> findPostsByAuthorIdsAndLastPostId(@Param("postAuthorIds") List<Long> postAuthorIds,
