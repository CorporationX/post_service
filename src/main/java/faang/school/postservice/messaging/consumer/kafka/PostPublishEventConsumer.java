...
public class PostPublishEventConsumer {
    ...
    public void listenPostEvent(PostPublishEvent event) {
        log.info("Рассылка подписчикам автора с id: {} нового поста", event.getAuthorId());
        if (event.getFollowersId().isEmpty()) {
            return;
        }
