...
@Component
public class AlbumMaxPostsFilter implements AlbumFilter {
    @Override
    ...
    public Stream<AlbumUpdateDto> apply(Stream<AlbumUpdateDto> albums, AlbumFilterDto filter) {
        Integer maxCount = filter.getMaxPostsCountPattern();

        return albums.filter(album -> album.getPostsIds().size() <= maxCount);
    }
}
