...
class ViewListenerIntTest {
    ...
    @Test
    @DisplayName("Успешное чтение ViewEvent для поста из Kafka")
    void positive_consumerViewPostEventReceived() {
        ViewEvent event = createEvent();
        String expected = jsonMapper.toJson(event);

        kafkaTemplate.send(kafkaProperty.topic().postView(), expected);

        verify(listener, timeout(5000).times(1)).consumeViewPost(dataCaptor.capture(), any(Acknowledgment.class));
