...
class PostListenerIntTest {
    ...
    @Test
    @DisplayName("Успешное чтение PostNewEvent из Kafka")
    void positive_consumerPostEventReceived() {
        PostNewEvent event = createEvent();
        String expected = jsonMapper.toJson(event);

        kafkaTemplate.send(kafkaProperty.topic().postNew(), expected);
